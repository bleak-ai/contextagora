...
**Relax integration-specific checks for workflows.** The validator's existing `info.md` section checks (Purpose, Auth & access, etc.) and the secrets/dependencies cross-checks are designed for integrations. For `kind: workflow` modules, skip those — workflow `info.md` is free-form prose explaining the workflow, not the integration template. The current validator does **not** dispatch on `kind` (it runs integration-shaped checks on every module today); part of this work is introducing that per-kind dispatch from scratch and routing the existing checks under an `integration` branch, the new structural checks (entry_step + steps/) under a `workflow` branch, and a universal branch (module.yaml parses, info.md exists, llms.txt exists) that runs for all kinds. Tasks should keep behaving as they do today (which is to say: only the universal checks apply meaningfully — the integration-shaped checks already produce harmless warnings on tasks that we'll silence by routing them under `integration` only).
