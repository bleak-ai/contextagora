"""Unit tests for src.services.claude.
...
def test_run_headless_argv_shape():
    ...
    with patch("src.services.claude.subprocess.run", return_value=mock_proc) as mock_run:
...
def test_run_headless_respects_timeout_kwarg():
    ...
    with patch("src.services.claude.subprocess.run",
...
def test_stream_minimal_argv():
    ...
    with patch("src.services.claude.subprocess.Popen", return_value=fake_popen) as mock_popen:
...
def test_stream_with_session_id():
    ...
    with patch("src.services.claude.subprocess.Popen") as mock_popen:
